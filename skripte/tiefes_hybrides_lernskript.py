...
def test_loop(test_schwarz, test_weiss, anzahl_tests, liste, 
    ...
    with torch.no_grad():
        ...
        if ergebnis_schwarz >= bestes_ergebnis[0]:
            ...
            torch.save(netz.state_dict(), 'tiefe_gewichte_hybrid_schwarz')
        ...
        if ergebnis_weiss >= bestes_ergebnis[1]:
            ...
            torch.save(netz.state_dict(), 'tiefe_gewichte_hybrid_weiss')
...
for z in range(anzahl_zyklen):
    ...
    indexlist = random.sample(range(len(tabelle.bewertung)), stichproben_groesse)
    liste_stellungen = []
    liste_bewertungen = []
    for i, paar in enumerate(tabelle.bewertung.items()):
        if i in indexlist:
            stellung_neu = np.frombuffer(paar[0], dtype=np.int8)
            liste_stellungen.append(stellung_neu)
            bewertung_neu = paar[1][0]/paar[1][1]
...
torch.save(netz.state_dict(), 'tiefe_gewichte_hybrid_final')
